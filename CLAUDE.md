...
- **SVG 주석에 `--` 를 쓰지 말 것.** XML 규칙 위반이라 파일 전체가 파싱 실패하고
  깨진 이미지로 뜬다. `--accent` 같은 CSS 변수명을 주석에 적었다가 물렸다.
  주석 자체를 넣지 않는 게 안전하다.
...
- **Free 플랜은 디스크가 휘발성**이다. 그래서 Supabase Storage 에 백업한다
  (`SUPABASE_URL`/`SUPABASE_SERVICE_KEY`, 아래 참고). 미설정이면 백업이 그냥
  꺼지고 로컬 파일로만 동작한다 — 로컬 개발은 지금과 똑같다.

### 원격 백업 (`web/src/persist.js`)

**원격은 durable 백업, 로컬 디스크는 핫 캐시.** `loadCache` 가 10곳에서 **동기로**
호출되므로 읽기 경로를 네트워크로 바꾸면 매 요청에 왕복이 붙는다. 그래서:

- 유저당 **한 번만** 원격 → 로컬로 내려받는다(hydrate). 두 번째부터는 Set 조회.
- 쓰기는 로컬 먼저, 원격 push 는 뒤에서. 실패해도 응답을 붙잡지 않는다.
- gzip 으로 보낸다 — 실측 **1691KB → 210KB (88% 감소)**.

**'없음'과 '못 읽음'을 반드시 구분한다.** 둘을 뭉뚱그리면 이렇게 된다:
원격 장애 → `loadState` 가 빈 상태 반환 → 유저가 새로 뽑음 → **그 빈 상태가
백업으로 올라가 멀쩡한 기록을 지운다.** 백업하려다 백업을 날리는 경로다.
그래서 복원 실패한 유저는 `degraded` 로 표시하고 그 세션엔 **아무것도 안 올린다.**

- 두 blob(cache·state)은 **병렬로** 받는다. 순차면 느린 원격에서 대기가 두 배다.
- HTTP 타임아웃 8초. 이 호출이 유저의 첫 요청을 붙잡고 있다.
- `objectPath` 는 SteamID 가 숫자인지 검사한다 — 경로 조작 차단.
- Supabase 무료 프로젝트는 **1주 무접속 시 일시정지**된다. 방문이 뜸하면 깨워야 한다.
